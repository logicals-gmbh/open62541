...
static void
beforeBrowseCallback(UA_Server *server, const UA_NodeId *nodeId, UA_BrowseDirection browseDirection) {
    if(!isServerInitialized)
        return;
    if(nodeId->identifier.numeric == UA_NS0ID_OBJECTSFOLDER) {
        static bool isFirstCall = true;
        if(isFirstCall && (browseDirection == UA_BROWSEDIRECTION_FORWARD)) {
            UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND,
                        "create subnode on the fly");
            /* create subnode on the fly */
...
static void
beforeReadCallback(UA_Server *server, const UA_NodeId *nodeId) {
    if(!isServerInitialized)
        return;
    static bool isFirstCall = true;
    if(isFirstCall) {
        UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "Before read callback (first call)");
        isFirstCall = false;
    }
}

static void
beforeWriteCallback(UA_Server *server, const UA_NodeId *nodeId) {
    if(!isServerInitialized)
        return;
    static bool isFirstCall = true;
    if(isFirstCall) {
        UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "Before write callback (first call)");
        isFirstCall = false;
    }
}
...
int main(void) {
    ...
    UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "Preparing server config");
    UA_ServerConfig config;
    memset(&config, 0, sizeof(UA_ServerConfig));
    (void)UA_ServerConfig_setDefault(&config);
    config.beforeBrowseCallback = beforeBrowseCallback;
    config.beforeReadCallback = beforeReadCallback;
    config.beforeWriteCallback = beforeWriteCallback;
    UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "Create server");
    UA_Server *server = UA_Server_newWithConfig(&config);
    UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "Created server");
